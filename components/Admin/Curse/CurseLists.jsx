...
const CurseForm = dynamic(() => import('./CurseForm'), {
    ssr: false
});

const CurseCard = dynamic(() => import('./CurseCard'), {
    ssr: false
});

class CurseLists extends BaseClass {
    constructor(props) {
        super(props);
        this.state.title = 'دوره ها';
                    ...
                    <FilterContainer show={this.state.filter} close={() => this.setState({filter: false})}>
                        <div className='row'>
                            ...
                            <CustomControl
                                type='select'
                                id='categoryId'
                                options={{'': 'همه دسته بندی ها', ...ArrayComboToObject(this.props.results.var.categories)}}
                                value={this.state.params.categoryId}
                                className={`custom-input col-md-3 col-6`}
                                onChange={this.onChange}
                            />
                            ...
                            <CustomControl
                                type='select'
                                id='userId'
                                options={{'': 'همه استاد ها', ...ArrayComboToObject(this.props.results.var.teachers)}}
                        ...
                        <div className='row'>
                            {
                                this.props.results && this.props.results.items.map((item, i) => {
                                    return (
                                        <div key={`g-${i}`} className='col-12 col-xl-6 mb-3'>
                                            <CurseCard
                                                categories={this.props.results.var.categories}
                                                teachers={this.props.results.var.teachers}
...
CurseLists.propTypes = {
    results: PropTypes.object,
    parameters: PropTypes.object
}


export default withRouter(CurseLists);